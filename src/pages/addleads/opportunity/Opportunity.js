...
const Opportunity = (props) => {
    ...
    const opportunityData = listTable.filter((item) => item.status === 'opportunity');
    const [selectedLeads, setSelectedLeads] = useState([]);
    const [showPopup, setShowPopup] = useState(false);
    const [user, setUser] = useState(''); //for one users

    const leadCodes = opportunityData.map((item) => item.leadCode);
...
const handleSelectAllClick = (event) => {
    if (event.target.checked) {
        const newSelecteds = opportunityData.map((n) => n.id);
...
const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - opportunityData.length) : 0;

const filteredUsers = applySortFilter(opportunityData, getComparator(order, orderBy), filterName);
                ...
                <Scrollbar>
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table>
                            ...
                            />
                            <TableBody>
                                {opportunityData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo,index) => {
