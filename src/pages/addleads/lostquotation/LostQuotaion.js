...
const LostQuotation = (props) => {
    const dispatch = useDispatch();
    const { lead } = props;
    const [listTable, setListTable] = useState(lead?.data);

    const lostquotationData = listTable.filter((item) => item.status === 'lostquotation');
    const [selectedLeads, setSelectedLeads] = useState([]);
    const [showPopup, setShowPopup] = useState(false);
    const [user, setUser] = useState(''); //for one users

    const leadCodes = lostquotationData.map((item) => item.leadCode);
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = lostquotationData.map((n) => n.id);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - lostquotationData.length) : 0;

    const filteredUsers = applySortFilter(lostquotationData, getComparator(order, orderBy), filterName);
        ...
        <>
            <Box sx={{ mt: 2 }}>
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table>
                            <UserListHead
                                ...
                                rowCount={lostquotationData.length}
                                numSelected={selected.length}
                                onRequestSort={handleRequestSort}
                                onSelectAllClick={handleSelectAllClick}
                            />
                            <TableBody>
                                {lostquotationData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo, index) => {
                                                ...
                                                <Stack direction="row" alignItems="center" spacing={2}>
                                                    <Dropdown>
                                                        ...
                                                        <Dropdown.Menu size="sm" title="">
                                                            <Dropdown.Item onClick={() => handleRestore(leadCode)}>
                                                                <RefreshIcon />
                                                                &nbsp;&nbsp;&nbsp; Restore
                                                            </Dropdown.Item>
                                                            <Dropdown.Item onClick={() => handleDelete(leadCode)}>
                                                                <DeleteOutlineIcon />
                                                                &nbsp;&nbsp;&nbsp; Delete
                                                            </Dropdown.Item>
                                                        </Dropdown.Menu>
                ...
                <TablePagination
                    ...
                    component="div"
                    count={lostquotationData.length}
