...
const Converted = (props) => {
    ...
    const [listTable, setListTable] = useState(lead?.data);
        const convertedData = listTable.filter((item) => item.status === 'converted');
        const [selectedLeads, setSelectedLeads] = useState([]);
        const [showPopup, setShowPopup] = useState(false);
        const [user, setUser] = useState(''); //for one users
    
        const leadCodes = convertedData.map((item) => item.leadCode);
...
const handleSelectAllClick = (event) => {
    if (event.target.checked) {
        const newSelecteds = convertedData.map((n) => n.id);
...
const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - convertedData.length) : 0;

const filteredUsers = applySortFilter(convertedData, getComparator(order, orderBy), filterName);
                ...
                <Scrollbar>
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table>
                            ...
                            />
                            <TableBody>
                                {convertedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo,index) => {
