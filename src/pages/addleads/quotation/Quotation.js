...
const Quotation = (props) => {
    ...
    const quotationData = listTable.filter((item) => item.status === 'quotation');
    const [selectedLeads, setSelectedLeads] = useState([]);
    const [showPopup, setShowPopup] = useState(false);
    const [user, setUser] = useState(''); //for one users

    const leadCodes = quotationData.map((item) => item.leadCode);
...
const handleSelectAllClick = (event) => {
    if (event.target.checked) {
        const newSelecteds = quotationData.map((n) => n.id);
...
const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - quotationData.length) : 0;

const filteredUsers = applySortFilter(quotationData, getComparator(order, orderBy), filterName);
                ...
                <Scrollbar>
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table>
                            ...
                            />
                            <TableBody>
                                {quotationData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo,index) => {
