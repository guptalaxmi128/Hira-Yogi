...
const Interested = (props) => {
    ...
    const interestedData = listTable.filter((item) => item.status === 'interested');
    const [selectedLeads, setSelectedLeads] = useState([]);
    const [showPopup, setShowPopup] = useState(false);
    const [user, setUser] = useState(''); //for one users

    const leadCodes = interestedData.map((item) => item.leadCode);
...
const handleSelectAllClick = (event) => {
    if (event.target.checked) {
        const newSelecteds = interestedData .map((n) => n.id);
...
const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - interestedData .length) : 0;

const filteredUsers = applySortFilter(interestedData , getComparator(order, orderBy), filterName);
                    ...
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table>
                            <UserListHead
                                ...
                                rowCount={interestedData.length}
                                numSelected={selected.length}
                                onRequestSort={handleRequestSort}
                                onSelectAllClick={handleSelectAllClick}
                            />
                            <TableBody>
                                {interestedData .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo,index) => {
