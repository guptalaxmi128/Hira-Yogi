...
const Lead = (props) => {
    ...
    const leadData = listTable.filter((item) => item.status === 'lead');

    const leadCodes = leadData.map((item) => item.leadCode);
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = leadData.map((n) => n.id);
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - leadData.length) : 0;

    const filteredUsers = applySortFilter(leadData, getComparator(order, orderBy), filterName);
                ...
                <Scrollbar>
                    <TableContainer sx={{ minWidth: 800 }}>
                        <Table>
                            ...
                            />
                            <TableBody>
                                {leadData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo, index) => {
